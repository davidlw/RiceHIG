...
class PPPileUpVertexFilterLowPU : public edm::one::EDFilter<> {
public:
    explicit PPPileUpVertexFilterLowPU(const edm::ParameterSet&);
    ~PPPileUpVertexFilterLowPU() override;
...
PPPileUpVertexFilterLowPU::PPPileUpVertexFilterLowPU(const edm::ParameterSet& iConfig)
    : vtxToken_(consumes<std::vector<reco::Vertex>>(iConfig.getParameter<edm::InputTag>("vtxSrc"))),
...
bool PPwayToSort(const reco::Vertex& a, const reco::Vertex& b) {
    return a.tracksSize() > b.tracksSize();
}

bool PPPileUpVertexFilterLowPU::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    using namespace edm;
    bool accepted = true;
    Handle<std::vector<reco::Vertex>> vcol;
    iEvent.getByToken(vtxToken_, vcol);
    ...
    for (unsigned int i = 1; i < vsorted.size(); i++) {
        ...
        if (doDzNtrkCut_) {
            ...
            if (nTrk >= (int)dzCutByNtrk_.size() && dz > dzCutByNtrk_[dzCutByNtrk_.size() - 1] && dz < dzTolerance_) {
                accepted = false;
            }
        }
        ...
        if (doSurfaceCut_) {
            ...
            if (nTrk > func2D_->Eval(surfaceMinDzEval_, nTrkLead) && dz <= surfaceMinDzEval_) {
                accepted = false;
            }
        }
    }
...
void PPPileUpVertexFilterLowPU::endJob() {
}
