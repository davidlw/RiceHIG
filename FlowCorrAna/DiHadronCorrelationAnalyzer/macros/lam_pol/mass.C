void mass()
{
  ...
  TFile* f_ks;
  TFile* f_lam;
  f_ks = new TFile("/net/hisrv0001/home/davidlw/scratch1/DiHadronCorrelations/outputs_312/HIData_Minbias_5TeV/merged/kspol_PbPb_sig_cut04_v8.root");
  f_lam = new TFile("/net/hisrv0001/home/davidlw/scratch1/DiHadronCorrelations/outputs_312/HIData_Minbias_5TeV/merged/lampol_PbPb_sig_cut_v7.root");
...
//  v40
  double ptbins[] = {0.0, 0.2, 0.4, 0.7, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0, 8.0, 12.0};
  double ptbinwidth[] = {0.2, 0.2,0.3,0.3,0.5,0.5,1.0,1.0,2.0,2.0,4.0};
...
//    sum_ks[i]->fitTo(*data_ks[i],Range("cut"));
    ...
    meanf  = mean_ks[i]->getVal();
    meanfe  = mean_ks[i]->getError();
    sigmaf1  = sigma1_ks[i]->getVal();
    sigmaf2  = sigma2_ks[i]->getVal();
    sigmaf1e  = sigma1_ks[i]->getError();
    sigmaf2e  = sigma2_ks[i]->getError();
    bkgf  = polysig_ks[i]->getVal();
    sigf1  = sig1_ks[i]->getVal();
    sigf2  = sig2_ks[i]->getVal();
    sigwf1  = sigf1 /(sigf1 +sigf2 );
    sigwf2  = sigf2 /(sigf1 +sigf2 );
    sigmaf  = sqrt(sigmaf1*sigmaf1*sigwf1  + sigmaf2*sigmaf2*sigwf2 );
    sigmafe  = sqrt(sigmaf1e*sigmaf1e*sigwf1  + sigmaf2e*sigmaf2e*sigwf2 );

    massmin  = meanf  - 2.0*sigmaf ;
    massmax  = meanf  + 2.0*sigmaf ;

    mean_pdg = 0.4976;
//    double massmin  = mean_pdg - 0.02 ;
//    double massmax  = mean_pdg + 0.02 ;

    nmin = ksmassall[i]->GetXaxis()->FindBin(massmin);
    nmax = ksmassall[i]->GetXaxis()->FindBin(massmax);
    totyhe = 0;
    totyh  = ksmassall[i]->IntegralAndError(nmin,nmax,totyhe);
    l1_ks[i] = new TLine(massmin ,0,massmin ,5000);
    l2_ks[i] = new TLine(massmax ,0,massmax ,5000);
    x_ks[i]->setRange("cut",massmin ,massmax );
    ibkg  = poly_ks[i]->createIntegral(*x_ks[i],NormSet(*x_ks[i]),Range("cut"));
    isig1  = gaus1_ks[i]->createIntegral(*x_ks[i],NormSet(*x_ks[i]));//,Range("cut"));
    isig2  = gaus2_ks[i]->createIntegral(*x_ks[i],NormSet(*x_ks[i]));//,Range("cut"));
    ibkgf = ibkg->getVal();
    bkgfe = polysig_ks[i]->getError();
    isig1f = isig1->getVal();
    isig2f = isig2->getVal();
    bkgy = ibkgf*bkgf ;
    bkgye = ibkgf*bkgfe ;
    ...
    sigyhe = sqrt(totyhe*totyhe+bkgye*bkgye);
    sigy1  = isig1f *sigf1 ;
    sigy2  = isig2f *sigf2 ;
    sigy  = sigy1  + sigy2 ;
